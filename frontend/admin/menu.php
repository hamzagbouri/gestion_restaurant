...
$result = $con -> query('SELECT * from plat');
        ...
        <aside class="w-[16rem] border-r min-h-full  flex flex-col items-center gap-16 ">
            ...
            <div class="">
                <div class="grid gap-4 w-[100%]">
                    <a href="index.php" class="flex gap-4 px-4 py-2 rounded-2xl"><img src="./img/home.svg" alt="">
                        Dashboard </a>
                    <a href='menu.php' class='flex gap-4 px-4 py-2 rounded-2xl'><img src='./img/briefcase.svg' alt=''>
                        Menu </a>
                    <a href='reservation.php' class='flex gap-4 px-4 py-2 rounded-2xl'><img id='btn-icon' class='mt-1'
                            src='./img/3 User.svg' alt=''> Reservations</a>
                </div>
            </div>
        </aside>
        <div class="grow">
            ...
            ?>
                ...
                <div class="flex justify-between items-center px-4 border py-4 rounded-lg">
                    <div class="flex gap-2">
                        ...
                        <input class="search outline-none border-none w-72 px-4 py-2 rounded-2xl" type="search"
                            name="search-input" id="search-input" placeholder="Search menu by name...">
                    </div>
                ...
                foreach($allMenu as $menu){
                    $menuId = $menu['id'];
                    $menuTitle = $menu['titre'];
                    $menuPrice = $menu['prix'];
                    $allPlat = $con->query("SELECT * from plat inner join menu_plat on menu_plat.id_plat = plat.id inner join menu on menu_plat.id_menu = menu.id where menu.id = $menuId");
                    echo "
                    <div class='w-[90%] border h-auto flex flex-col gap-2 rounded-lg p-2'>
                        <div class='flex justify-between'>
                            <p class='text-center'>$menuTitle</p>
                            <div class='flex gap-2'>
                ...
                foreach ($allPlat as $plat) {
                    ...
                    <div class='flex border p-1'>
                        <div class='w-[70%]'>
                         <p><span class='text-gray-500'>Plat:</span> {$plat['titre_plat']}</p>
                            <h3><span class='text-gray-500'>Categorie: </span>{$plat['categorie']}</h3>
                ...
                echo "
                    <p class='text-center'><span class='text-gray-500'>Menu Price: </span>$menuPrice $</p>
                    </div>
                ";
        ...
        <div id='modal' class='modal bg-black bg-opacity-75 hidden items-center justify-center fixed inset-0 z-50'>
            <div
                ...
                <div class="flex flex-col p-4">
                    ...
                    <form id="menu-form" method="post" action="../../backend/actionsPHP/menu/add.php"
                        enctype="multipart/form-data" class="flex flex-col pt-16 gap-4">

                        <div class="flex flex-col">
                            <label for="menu-title">Menu Title</label>
                            <input class="border border-gray-200 border-2 rounded-lg p-2" type="text" id="menu-title"
                                name="menu-title">
                        </div>

                        <div class="flex flex-col">
                            <label for="menu-price">Price</label>
                            <input class="border border-gray-200 border-2 rounded-lg p-2" type="number" id="menu-price"
                                name="menu-price">
                        </div>
                        <div class="flex flex-col">
